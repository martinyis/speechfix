...
export default function VoiceSessionScreen() {
  ...
  const handleComplete = useCallback((displayName: string | null, speechObservation: string | null) => {
    useOnboardingStore.getState().setOnboardingResult(displayName, speechObservation);
    useAuthStore.getState().setOnboardingComplete();
    router.replace('/(onboarding)/analysis-reveal');
  }, []);

  const handleError = useCallback((message: string) => {
    console.warn('Onboarding voice error:', message);
    // On error, still complete onboarding so user isn't stuck
    useAuthStore.getState().setOnboardingComplete();
