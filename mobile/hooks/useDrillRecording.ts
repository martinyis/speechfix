...
export function useDrillRecording() {
  const [state, setState] = useState<DrillRecordingState>('idle');
  ...
  const submitRecording = useCallback(async (
    ...
    try {
      ...
      // Text fields MUST come before the file for @fastify/multipart
      formData.append('weakSpotId', String(weakSpotId));
      formData.append('itemType', drillItem.type);
      formData.append('itemId', String(drillItem.data.id));
      ...
      const res = await fetch(`${API_BASE_URL}/practice/weak-spot-evaluate`, {
        method: 'POST',
      ...
      const data: DrillResult = await res.json();
      setResult(data);
      setState('result');
    } catch {
    ...
    try {
      ...
      const { subscription } = await ExpoPlayAudioStream.startRecording({
        ...
        onAudioStream: async (event: any) => {
          if (event.data) {
            // Drop first chunk (mic init noise)
  ...
  const stop = useCallback(async (
    ...
    await submitRecording(drillItem, weakSpotId);
